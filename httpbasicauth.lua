...
function _M.new(config)
	...
	-- ngx.ctx.user_authenticated = false
        user_authenticated = false -- Just for safety. The real user_authenticated will be checked at _M:rewrite()
...
function _M:rewrite()
        ...
        if auth_header ~= nil and auth_header:find(" ") ~= nil then
                ...
                if not user_authenticated then 
                        ngx.log(ngx.NOTICE, 'NO MATCH found for auth_configured: ', auth_configured)
                        ngx.status = 403
                        ngx.log(ngx.ERR, "user_authenticated KO. Sending back HTTP 403 : HTTP Basic auth credentials does NOT match")
                        ngx.say("HTTP Basic auth credentials does NOT match")
                else
                        ngx.log(ngx.INFO, "user_authenticated OK")
                end
